...
class Day2Manager: ObservableObject {
    @Published public var sum = 0
    
    private var maxRed = 12
    private var maxBlue = 14
    private var maxGreen = 13
    
    var data: [Game] {
        var games: [Game] = []
        for g in string {
            guard let game = loadData(initialGameString: g) else {
                return []
            }
            games.append(game)
        }
    ...
    private func loadData(initialGameString: String) -> Game? {
        let game: String = String(initialGameString.split(separator: ":").first ?? "")
        let turns = initialGameString.split(separator: ":").last?.split(separator: ";")
        
        guard let id = Int(String(game.split(separator: " ").last ?? "")) else {
            return nil
        }
        
        guard let turn = turns else {
            return nil
        }
        var realGame = Game(id: id)
        
        guard let realTurns: ArraySlice<Substring> = turn.split(separator: ",").first else {
            return nil
        }
        
        for index in 0..<realTurns.count {
            var turnToAppend = Turn()
            let RT = String(realTurns[index])
            let cube = RT.split(separator: ",")
            for i in 0..<cube.count {
                let d = String(cube[i])
                guard let number = Int(String(d.split(separator: " ").first ?? "")) else { return nil
                }
                let color = String(d.split(separator: " ").last ?? "")
                let cube = Cube(cuantity: number, color: CubeColor(rawValue: color) ?? CubeColor.blue)
                turnToAppend.cubes.append(cube)
            }
            realGame.turns.append(turnToAppend)
        }
